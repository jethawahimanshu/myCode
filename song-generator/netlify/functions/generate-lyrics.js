...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    prompt += `\nIMPORTANT: Keep lyrics SHORT and CONCISE (maximum 500 characters total).\n\n`;
    prompt += `Please provide:\n`;
    prompt += `1. A creative song title\n`;
    prompt += `2. Compact lyrics with LOWERCASE structure tags:\n`;
    prompt += `   - Use [verse], [chorus], [bridge] tags (lowercase!)\n`;
    prompt += `   - Keep each section to 2-3 short lines\n`;
    prompt += `   - Maximum 500 characters total for all lyrics\n`;
    ...
    prompt += `  "lyrics": "Lyrics with [verse], [chorus] tags (lowercase), max 500 chars",\n`;
